...
<body>
 ...
 $MAIN_SITE_URL = "http://superpilot.in/dev/tutors/";
...
if(!empty($sqltable[0]['current_order_id']))
								{ ?>
...
<div class="container">
    ...
    <div class="col-md-4 col-4">
     ...
     <a href="orderlist.php?userid=<?= $_GET['userId'] ?>" > <button class="btn btn-success btn-sm" style="font-size:10px;" type="button">
         HISTORY
      </button></a>
      <?php } ?>
    </div>
       ...
       for($fct=0;$fct<count($foodCat);$fct++){
        ...
        <div class="col-md-4 col-sm-6 col-6 mx-auto">
          <div class="each-item">
            ...
            <img src="<?= $MAIN_SITE_URL ?>web/uploads/productimages/<?= $imgArr[$fcid]; ?>">
            <?php }else { ?>
            ...
            <div class="d-flex">
            <div class="add-btn mx-auto" id="fcaddid_<?= $fcid; ?>" onclick = "openfoodqtypopup('<?= $fcid; ?>')" >ADD</div>
            ...
            <div class="add-btn text-center col-5" onclick="productorder('<?= $fcid ?>',1,<?= $pricePlainQtyArr[$foodCat[$fct]]; ?>,'<?= $fcqtyuniqueidArr[$fcid]; ?>')" id="fcaddid_<?= $fcid ?>" >ADD</div>
            <div class="hide-btn" id="incdec_<?= $fcid ?>">
              <span class="quantity">
...
<button id="place_order" class="btn btn-success text-black w-100 m-auto" onclick="placeorder()"><b>Place Order</b></button>
  </form>
 </div>
</div>
</div>
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-7 col-7 text-right">
         <button class="btn btn-sm btn-info cart-pop"><b>Place Order</b></button>
...
<div class="modal fade pop-itemlist w-100" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog pop-center" role="document">
    ...
    <a href="#" class="quantity__plus"><span>+</span></a>
      ...
      <button type="button" id="amount" class="btn btn-sm mt-3 btn-success" data-dismiss="modal">Submit</button>
    </div>
      </div>
...
function openfoodqtypopup(fqty){
	...
	for(var f=0;f< foodQtyArr.length;f++)
    {
		...
		msg += '<div class="custom-control custom-checkbox d-block w-100 my-2">\
        <input type="checkbox" name="foodcate" class="custom-control-input" onclick="toaddprice(\''+fqty+'\','+itemQtyPrice+',\''+itemUniqueId+'\')" id="customCheck'+foodQtyArr[f]+'" value="'+foodQtyArr[f]+'">\
        <label class="custom-control-label ft-label d-flex" for="customCheck'+foodQtyArr[f]+'">\
        ...
        msg +=  '<div class="text-center"><button type="button" id="amount" onclick="productorder(\''+fqty+'\',2,null)"  class="btn btn-sm mt-3 btn-success" data-dismiss="modal">Submit</button></div>\
...
function caltax(productid = null){
...
function grnadTotals(currentGrandTotalAmt,tipchange = ''){
			...
			if(tipchange == 1){
			    ...
			    var cart_tip = ((parseFloat(merchantdet[0]['tip'])/100)*parseFloat(currentGrandTotalAmt)).toFixed(2);    
			}
...
$("#prodd_id_"+catname).remove();
$("#item_tot_price_id_"+catname).remove();
